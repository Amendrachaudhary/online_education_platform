...
const Player = () => {
	...
	const getCourseData = () => {
		...
		// If not found, try to fetch from API or use dummy data
		(async () => {
			try {
				const { dummyCourses } = await import('../../assets/assets');
				const dummyCourse = dummyCourses.find((course) => course._id === courseId);
				if (dummyCourse) {
	...
	useEffect(() => {
		// Always try to get course data (works with or without enrollments in demo mode)
		getCourseData();
	}, [courseId]);

	const markLectureAsCompleted = async (lectureId) => {
		// Demo mode - just show success
		toast.success("Demo Mode: Lecture marked as completed!");
		// In real mode, update progress
		try {
			const token = await getToken();
	...
	const getCourseProgress = async () => {
		...
		try {
			...
			// Demo mode - use empty progress
		}
	};

	const handleRate = async (rating) => {
		// Demo mode - just show success
		toast.success(`Demo Mode: Rated ${rating} stars!`);
