...
const CourseDetails = () => {
	...
	const fetcheCourseData = async () => {
		// Try to find in allCourses first (from context)
		...
		try {
			...
			if (data.success && data.courseData) {
				...
				// Try to find in dummy courses
				const { dummyCourses } = await import('../../assets/assets');
				const dummyCourse = dummyCourses.find((course) => course._id === id);
				if (dummyCourse) {
	...
	const enrollCourse = async () => {
		try {
			...
			// Demo mode: Add course to enrolled courses in context
			if (!userData || !userData.user_id || userData.user_id === 'demo-user') {
				// Add to enrolled courses in context
				if (courseData && !enrolledCourses.find(c => c._id === courseData._id)) {
					setEnrolledCourses([...enrolledCourses, courseData]);
					setIsAlreadyEnrolled(true);
				}
				toast.success("Demo Mode: Successfully enrolled in course!");
				setTimeout(() => {
			...
			// In demo mode, add to enrolled courses
			if (courseData && !enrolledCourses.find(c => c._id === courseData._id)) {
				setEnrolledCourses([...enrolledCourses, courseData]);
				setIsAlreadyEnrolled(true);
			}
			toast.success("Demo Mode: Successfully enrolled!");
			setTimeout(() => {
				navigate('/my-enrollments');
			}, 1500);
		}
